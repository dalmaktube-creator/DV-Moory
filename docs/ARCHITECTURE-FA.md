...
- `moory_tool_catalog` پروفایل‌های کوچک‌تر `core`، `git` و `github` را برای کشف ابزارها معرفی می‌کند، اما ابزارها را به‌صورت پویا مخفی نمی‌کند.
- `validate_project` فایل‌های Python، JSON و TOML را بدون اجرای کد مخزن parse می‌کند.
- `apply_change_set` فقط تغییر فایل‌های tracked را تراکنشی اعمال می‌کند، اعتبارسنجی static را پیش‌فرض می‌گیرد و در شکست rollback انجام می‌دهد.
